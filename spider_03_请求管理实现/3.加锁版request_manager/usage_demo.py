...
from reuquest_manager.request import Request
from reuquest_manager import RequestScheduler

REQUEST_SCHEDULER_CONFIG = {
    "queue_type": "fifo",
    # 如何配置需查看redis队列的实例化时需要的参数  redis_queue模块中
    # 注意：redis的key会被queue_name重写，因此此处不用写
    "queue_kwargs": {"host": "192.168.219.3", "port": 6379},
    ...
    # 是否在请求去重判断时使用锁
    "use_lock": True,
    "redis_lock_config": {"host": "192.168.219.3"}
}
