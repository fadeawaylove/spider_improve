...
class RequestScheduler(object):
    ...
    def __init__(
        ...
        self._redis_lock_config = redis_lock_config
        self._locks = {}
    ...
    def _set_queue_cls(self, queue_type):
        ...
        from reuquest_manager.utils.redis_tools import get_redis_queue_cls
    ...
    def add_request(self, request_obj, queue_name, filter_name=None):
        ...
        if self._use_lock is False:
            ...
            # 使用redis分布式锁控制请求去重判断时的资源竞争问题

            # 每一个过滤器对应单独的一把锁
            lock_name = "lock-" + filter_name
            if lock_name not in self._locks:
                from .utils.redis_tools import get_redis_lock_cls
                Lock = get_redis_lock_cls()
                self._redis_lock_config["lock_name"] = lock_name
                self._locks[lock_name] = Lock(**self._redis_lock_config)
